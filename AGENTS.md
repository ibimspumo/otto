...
- **Präsenz statt Programm**: Otto lebt im Tray (kein Dock-Icon, `ActivationPolicy::Accessory`). Die Insel (Fenster `main`, kompakt 280×104, erweitert 560×104) ist transparent außerhalb der Kapsel — sichtbar sind nur 3D-Kern, Caption-Zeile (aktuelle Aktivität) und Hover-Controls. KEINE Transkript-/Textausgabe (bewusste Nutzer-Entscheidung). Er startet unsichtbar und wird per Hotkey/Wake-Word/Tray gerufen; Esc/Hotkey = Dismiss (trennt UND versteckt).
- **Ergebnisse fliegen als Peek-Karte ein**: Erstellt Otto ein Artefakt (Bild, Suche, Dokument), erscheint das Panel-Fenster (Label `panel`, eigene React-Wurzel `PanelApp.tsx` via `index.html?panel=1`) zuerst klein unten links als Drop-Stapel (kein Fokus-Klau, wie ein macOS-Screenshot-Thumbnail) — Klick vergrößert auf Quick Look (zentriert, Vibrancy-Glas, Größe nach Inhalt). Einstellungen & Persona-Dateien gibt es NUR über das Tray-Menü (immer full); das Panel selbst hat keine Tabs. Orchestrator bleibt App.tsx: Zustand per Events (`panel-state`, `panel-open`), Aktionen zurück (`panel-action`, `panel-close`, `settings-changed`, `style-changed`). Fenster-Helfer in `lib/hudWindow.ts`.
- **Zustandsfarben konsequent**: Bernstein = spricht, Eisblau = hört, Violett = denkt. Die Zustandsfarbe strahlt als Aura hinter der Insel (`.island-stage[data-state]`).
...
- **Rust** `src-tauri/src/`: `lib.rs` (Settings, Agent-Dateien mit Default-Seeding, Brave Search, abgesichertes `run_terminal`, Setup: Accessory-Policy + Tray + Vibrancy), `shell_safety.rs` (gemeinsame Blockliste/Risikoprüfung für direkte Shell-Befehle), `tray.rs` (Tray-Icon zur Laufzeit gezeichnet als Template-Image, Menü → Events `tray-toggle`/`tray-connect`/`tray-settings`), `sessions.rs` (SQLite + FTS5 unter `sessions.db`: Transkript-Protokolle, `processed`-Flag, Volltextsuche, Cleanup; verwaiste Sessions werden nach 6 h als beendet markiert), `memory.rs` (Tagesnotizen `agent/memory/YYYY-MM-DD.md`, `state.json` für Konsolidierungs-Zeitstempel), `skills.rs` (Skills `agent/skills/*.md`, Frontmatter-Parsing für Progressive Disclosure), `images.rs` (persistente Galerie), `computer_use.rs` (Responses API mit gpt-5.5, Abbruch über `cu_cancel`), `cli.rs` (Jobs: Codex/Codex CLI **und abgesicherte `shell`** via `zsh -lc` in eigener Prozessgruppe; Events `cli-line`/`cli-done`), `wake.rs` (Wake Word über NSSpeechRecognizer).
...
`~/Library/Application Support/de.agentz.otto/` → `settings.json` (macOS: API-Keys liegen in der Keychain, nicht im JSON),
